...
func toBeAccountedLabelsCount(g *Gmail, labels []string) int64 {
	...
	// then the label count is 1.
	if len(g.label) != 0 {
		return 1
	}
	// Else no label was specified so return the
	// valid labels count.
	count := int64(0)
...
func getMailCount(g *Gmail) (int64, error) {
	totalCount := int64(0)
	...
	for _, label := range labelsResponse.Labels {
		...
		to_account := int64(fullLabel.MessagesTotal)
		...
		// constructor: add to the total account of mails
		// matching this loop's label excepted if the are in 1)
		// above where we withdraw the account.
		if len(g.labelId) == 0 && isToBeAccounted(label.Name) {
			totalCount += to_account
...
func (g *Gmail) full() error {
	...
	t, err := getMailCount(g)         // Total count, for progress reporting
	if err != nil {
		return err
	}
